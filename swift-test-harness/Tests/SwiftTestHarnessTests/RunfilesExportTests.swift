...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Runfiles.swiftmodule/`
...
//      `import Runfiles` above would fail at compile time.
//
//   2. The static archive `libRunfiles.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Runfiles" }` and
//      `flattenPackage = "io.github.kotlinmania.runfiles"` configuration in
...
//      via the `RunfilesLibrary` product.
//
...
final class RunfilesExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Runfiles swift module imported cleanly")
    }
}
