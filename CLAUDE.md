# Claude Code Project Instructions — runfiles-kotlin

## Project Overview

This is **runfiles-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`runfiles`](https://crates.io/crates/runfiles).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/runfiles/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/runfiles>
