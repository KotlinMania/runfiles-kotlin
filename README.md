...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`runfiles`](https://crates.io/crates/runfiles).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `runfiles` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Bazel runfiles loader
...
Original work copyrighted by the upstream `runfiles` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
